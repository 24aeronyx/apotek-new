...
import { ShoppingCart, Search, Plus, Minus, Trash2, CreditCard, CheckCircle, Printer, Tag, Banknote, AlertTriangle } from 'lucide-react';
    ...
    setCart((prevCart) => {
      ...
      return [
        ...prevCart,
        {
          ...
          name: drug.name,
          unit: drug.unit,
          unit_price: Number(drug.selling_price),
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-5.5rem)] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-4 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          {loading ? (
            <div className="text-center py-20 text-slate-400 text-xs">Memuat katalog obat...</div>
          ) : filteredDrugs.length === 0 ? (
            <div className="text-center py-20 text-slate-400 text-xs">Obat tidak ditemukan.</div>
          ) : (
                  ...
                  >
                    <div>
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-[10px] font-mono font-bold text-slate-400">{drug.code}</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                          !isOutOfStock ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'
                        }`}>
                          Stok: {drug.total_stock}
                        </span>
                      </div>
                      <h4 className="font-bold text-slate-800 text-xs line-clamp-2 leading-snug">{drug.name}</h4>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="pt-2 border-t border-slate-100 flex justify-between items-center">
                        ...
                        <span className="text-[10px] text-slate-400">/{drug.unit}</span>
                      </div>
                    </div>
                  </button>
                );
      ...
      <div className="w-full lg:w-96 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">Kasir Apotek</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 divide-y divide-slate-100">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.drug_id} className="pt-3 first:pt-0 flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => updateQty(item.drug_id, -1)} className="p-1 text-slate-500 hover:bg-slate-100 rounded-lg">
                  ...
                  <button onClick={() => removeItem(item.drug_id)} className="p-1 text-red-400 hover:bg-red-50 rounded-lg ml-1">
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50/50 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Bayar (Cash)</label>
              <input
                type="number"
                value={payAmount}
                onChange={(e) => setPayAmount(e.target.value)}
                className="w-full p-2 border border-slate-300 rounded-xl text-xs font-bold bg-white text-right"
                placeholder="0"
              />
            </div>
          </div>
      ...
      {/* ================= MODAL STRUK DETAIL (THERMAL THERMAL STANDAR) ================= */}
      {receiptData && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-xs w-full p-6 shadow-2xl border border-slate-200 text-xs font-mono">
            ...
            <div className="text-center space-y-1 pb-3 border-b border-dashed border-slate-300">
              <h3 className="font-bold text-slate-900 text-base font-sans tracking-wide">APOTEK SEHAT BERSAMA</h3>
              <p className="text-[10px] text-slate-500 font-sans">SIPA: 446/001/SIPA/2026</p>
              <p className="text-[10px] text-slate-500 font-sans">Jl. Kesehatan No. 123, Kota</p>
              <div className="pt-2 text-[10px] text-slate-600 text-left space-y-0.5 font-mono">
                <p className="flex justify-between"><span>No. Inv:</span> <strong className="text-slate-800">{receiptData.invoice_number}</strong></p>
                <p className="flex justify-between"><span>Tgl/Waktu:</span> <span>{new Date(receiptData.created_at || Date.now()).toLocaleString('id-ID')}</span></p>
                <p className="flex justify-between"><span>Kasir:</span> <span>{receiptData.cashier?.name || 'Admin'}</span></p>
              </div>
            </div>
            ...
            <div className="py-3 space-y-2 border-b border-dashed border-slate-300 max-h-48 overflow-y-auto print:max-h-none print:overflow-visible">
              {receiptData.sale_items?.map((item, idx) => (
                <div key={idx} className="space-y-0.5">
                  <p className="font-bold text-slate-800 truncate">{item.drug?.name}</p>
                  <div className="flex justify-between text-slate-600 text-[11px]">
                    <span>{item.qty} x Rp {Number(item.unit_price).toLocaleString('id-ID')}</span>
                    <span className="font-semibold text-slate-800">Rp {Number(item.subtotal).toLocaleString('id-ID')}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="py-3 border-b border-dashed border-slate-300 space-y-1">
              <div className="flex justify-between text-slate-600">
                <span>Subtotal:</span>
                <span>Rp {Number(receiptData.total_amount || receiptData.final_amount).toLocaleString('id-ID')}</span>
              </div>
              
              {Number(receiptData.discount_amount) > 0 && (
                <div className="flex justify-between text-emerald-600">
                  <span>Diskon:</span>
                  <span>- Rp {Number(receiptData.discount_amount).toLocaleString('id-ID')}</span>
                </div>
              )}

              <div className="flex justify-between font-bold text-slate-900 text-sm pt-1">
                <span>Total Akhir:</span>
                <span className="text-emerald-700">Rp {Number(receiptData.final_amount).toLocaleString('id-ID')}</span>
              </div>
                ...
                {receiptData.payment_method === 'CASH' && (
                  <>
                    <div className="flex justify-between">
                      ...
                      <span>Rp {Number(receiptData.pay_amount || receiptData.final_amount).toLocaleString('id-ID')}</span>
                    </div>
                    <div className="flex justify-between font-bold text-slate-800">
                      <span>Kembalian:</span>
                      <span>Rp {Number(receiptData.change_amount || 0).toLocaleString('id-ID')}</span>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="pt-3 text-center text-[10px] text-slate-400 space-y-1 font-sans">
              <p className="font-semibold text-slate-600">-- Terima Kasih & Semoga Lekas Sembuh --</p>
